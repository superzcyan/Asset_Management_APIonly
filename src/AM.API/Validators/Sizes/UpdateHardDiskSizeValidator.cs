...
{
    public class UpdateHardDiskSizeValidator : AbstractValidator<UpdateHardDisk>
    {

        private readonly DataContext _context;

        public UpdateHardDiskSizeValidator(DataContext dataContext, bool runDefaultValidations = true)
        {
        ...
        {
            if (_context.HardDiskSizes.Where(x => x.Id == id).Count() != 1)
            {
                return false;
            }
        ...
        private bool SizeNotYetRegistered(UpdateHardDisk model)
        {
            if (_context.HardDiskSizes
                        .Where(
                                    x =>
